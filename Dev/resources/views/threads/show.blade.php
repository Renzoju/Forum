    ...
    <div class="max-w-5xl mx-auto py-8 px-4">
        ...
        <div class="bg-white p-6 rounded shadow mb-6">
            ...
            <div class="text-sm text-gray-500">
                Gemaakt door: <strong>{{ $thread->user?->username ?? 'Onbekend' }}</strong>
            ...
            @if($thread->topics->count() > 0)
                @foreach($thread->topics as $topic)
                    <div class="border-b border-gray-200 pb-4 mb-4">
                        ...
                        <p class="text-gray-700 truncate">{{ Str::limit($topic->body, 120, '...') }}</p>
                        <div class="text-sm text-gray-500 mt-1">
                            Door: {{ $topic->user?->username ?? 'Onbekend' }} • {{ $topic->created_at->diffForHumans() }}
                        </div>

                        @auth
                            @if(Auth::id() === $topic->user_id || Auth::user()->isAdmin())
                                <div class="mt-2 flex space-x-3">
                                    <a href="{{ route('topics.edit', ['threadId' => $thread->thread_id, 'topicId' => $topic->topic_id]) }}" class="text-blue-500 hover:underline">Bewerken</a>
